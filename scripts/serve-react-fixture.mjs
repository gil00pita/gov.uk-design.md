...
const fixtureRoot = resolve(repositoryRoot, 'fixtures/react')
const govukRoot = resolve(repositoryRoot, 'node_modules/govuk-frontend/dist/govuk')
const port = Number.parseInt(process.env.GOVUK_REACT_FIXTURE_PORT ?? '4174', 10)
...
if (!Number.isInteger(port) || port < 1 || port > 65_535) {
  throw new Error('GOVUK_REACT_FIXTURE_PORT must be an integer from 1 to 65535.')
}
...
server.listen(port, '127.0.0.1', () => {
  console.log('React fixture available at http://127.0.0.1:' + port)
})
