...
import { renderFixturePage } from '../fixtures/angular/page.mjs'

const projectRoot = resolve(fileURLToPath(new URL('..', import.meta.url)))
const packageJson = JSON.parse(await readFile(resolve(projectRoot, 'package.json'), 'utf8'))
const html = await renderFixturePage()

test('Angular fixture pins its evidence dependencies and server-renders the page shell', () => {
  for (const dependency of [
    ...
    assert.equal(packageJson.devDependencies[dependency], '22.1.6')
  }
  ...
  assert.match(html, /src="\/angular\/client\.js"/)
  assert.match(html, /<govuk-fixture[^>]*ngh=/)
  assert.match(html, /<script id="ng-state" type="application\/json">/)
  assert.match(html, /"__nghData__"/)
})

test('Angular server output preserves the three vertical-slice component contracts', () => {
...
test('Angular server output has unique and resolvable accessibility IDs', () => {
  const ids = [...html.matchAll(/\bid="([^"]+)"/g)].map((match) => match[1])
  assert.equal(new Set(ids).size, ids.length, 'Angular fixture contains duplicate IDs')
...
test('Angular server output exposes Accordion content without JavaScript', () => {
  const contentRegions = [
    ...html.matchAll(/<div\b(?=[^>]*\bclass="govuk-accordion__section-content")(?=[^>]*\bid="angular-accordion-guidance-content-[^"]+")([^>]*)>/g)
  ]
