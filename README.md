...
Generated instruction adapters cover Codex and other `AGENTS.md` consumers, Claude Code, Gemini CLI, GitHub Copilot and Cursor. The package also includes task-based agent evaluations, conflict-safe install/update flows, a behaviour-tested Plain HTML/CSS reference adapter, experimental behaviour-tested React, Angular, Svelte and Astro adapters, and experimental behaviour-tested Tailwind CSS, shadcn/Radix and Chakra UI mappings. The generated [compatibility matrix](ui-frameworks/compatibility.md) states the evidence boundary for every UI adapter. See [PLAN.md](PLAN.md) for delivery status and restart instructions.
...
ui-frameworks/                   generated utility and component-library mappings
...
npm run fixture:serve:tailwind # Tailwind token bridge at http://127.0.0.1:4178
npm run fixture:serve:shadcn # shadcn/Radix SSR and hydration at http://127.0.0.1:4179
npm run fixture:serve:chakra # Chakra UI SSR and hydration at http://127.0.0.1:4180
```

The fixtures are development-only and are not included in the npm package. The Angular 22 fixture requires a Node version supported by Angular 22 (`^22.22.3`, `^24.15.0`, or `>=26.0.0`), and the Astro 7 fixture requires Node `>=22.12.0`. Tailwind and shadcn intentionally omit Tailwind Preflight. Chakra uses a reset-free, layer-free system and a static service-wrapper fallback. All three keep GOV.UK Frontend responsible for official page and component styles.

Automation commands support the stable-release gates:

```bash
npm run upstream:check       # compare the pinned release and 13/37/30 inventories
npm run changes:report       # render a deterministic generated-output diff
npm run release:provenance -- --allow-dirty # inspect a local candidate
```

Pull-request and tag workflows run the schema, source, link, installer, upgrade, fixture, accessibility, browser and package gates. The scheduled monitor opens a report-only pull request when upstream drifts; canonical records still require human review.
...
```bash
node bin/govuk-design-md.js init --target /path/to/project --framework react --ui tailwind,shadcn,chakra --ai codex,cursor
...
- Selected UI-framework guidance is installed under `ui-frameworks/` with a selection-filtered compatibility matrix. Tailwind maps reviewed utility values, shadcn uses locally owned Radix composition with explicit official-markup fallbacks, and Chakra supplies a reset-free token system for service-owned wrappers. None replaces official GOV.UK component classes, CSS, JavaScript or conformance testing.
- `.govuk-design-md.json` records every managed path, SHA-256 hash, and selected application-framework, UI-framework and AI adapter ID.
...
The original raw website extraction informed the baseline but is no longer kept in the maintained tree. Reviewed source manifests, canonical records and compact browser smoke evidence remain; official guidance and the pinned `govuk-frontend` release outrank crawler inference. The current baseline is `govuk-frontend@6.5.0`; see [sources/govuk.json](sources/govuk.json). AI adapter formats and their official documentation are recorded in [sources/ai-adapters.json](sources/ai-adapters.json). Application-framework compatibility claims are recorded in [sources/framework-adapters.json](sources/framework-adapters.json), and UI-framework claims are recorded separately in [sources/ui-framework-adapters.json](sources/ui-framework-adapters.json). Release-candidate provenance binds the package, Git commit, lockfile, source manifest, reviewed upstream and package-size measurements; registry publication must still use npm trusted publishing or `npm publish --provenance`.
